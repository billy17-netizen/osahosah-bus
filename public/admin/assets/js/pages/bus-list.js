...
document.getElementById("busList") &&
    (busList = new List("busList", options).on("updated", function (e) {
...
(xhttp.onload = function () {
    ...
    Array.from(e).forEach((e) => {
        busList.add({
            id:
                '<a href="javascript:void(0);" class="fw-medium link-primary">#BZ' +
                e.id +
                "</a>",
            bus_number: e.bus_number,
            capacity: e.capacity,
            bus_name: e.bus_name,
            image_url: e.image_url,
            model: e.bus_detail.model,
            color: e.bus_detail.color,
            manufacturing_year: e.bus_detail.manufacturing_year,
            wifi: e.bus_detail.wifi ? "Yes" : "No",
            ac: e.bus_detail.ac ? "Yes" : "No",
            dinner: e.bus_detail.dinner ? "Yes" : "No",
            about_the_bus: e.bus_detail.about_the_bus,
            essentials: e.bus_detail.essentials,
            snacks: e.bus_detail.snacks,
            safety_features: e.bus_detail.safety_features,
        });
        busList.sort("id", { order: "desc" }), refreshCallbacks();
    }),
        busList.remove(
            "id",
    ...
    xhttp.open("GET", "/api/admin/bus "),
    xhttp.send();
var isValue = (isCount = new DOMParser().parseFromString(
        busList.items.slice(-1)[0]._values.id,
        "text/html",
    ...
    busNumberField = document.getElementById("bus-number-field"),
    capacityField = document.getElementById("capacity-field"),
    busNameField = document.getElementById("bus-name-field"),
    ...
    manufacturingYearField = document.getElementById(
        "manufacturing-year-field",
    ),
    wifiField = document.getElementById("wifi-field"),
    acField = document.getElementById("ac-field"),
    dinnerField = document.getElementById("dinner-field"),
    aboutTheBusField = document.getElementById("about-the-bus-field"),
    essentialsField = document.getElementById("essentials-field"),
    snacksField = document.getElementById("snacks-field"),
    safetyFeaturesField = document.getElementById("safety-features-field"),
...
refreshCallbacks(),
    ...
    document.querySelector("#busList").addEventListener("click", function () {
        ischeckboxcheck();
    });
                        ...
                        ? (e.preventDefault(),
                          ...
                          editlist
                              ? "" !== customerNameField.value &&
                                ...
                                "" !== phoneField.value &&
                                editlist &&
                                ((t = busList.get({ id: idField.value })),
